// frontend/src/pages/campaign/GroupsPage.tsx
...
import groupsService, { type Group, type GroupFiltersDto, type GroupStatsDto } from '../../services/groupsService';

export const GroupsPage: React.FC = () => {
  const [view, setView] = useState<'list' | 'charts'>('list');
  const [filters, setFilters] = useState<GroupFiltersDto>({});
  ...
  const loadStats = useCallback(async () => {
    ...
    try {
      const statsData = await groupsService.getStats(filters);
      setStats(statsData);
    } catch (error: any) {
  ...
  const handleExport = async () => {
    try {
      setIsLoading(true);
      await groupsService.exportToExcel(filters);
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
            ...
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600">
                  Gestión de grupos y estructuras de campaña
                </p>
              </div>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        {stats && (
          <div className="mb-6">
            <GroupsStats stats={stats} isLoading={isLoading} />
          </div>
        )}
        ...
        {view === 'charts' && stats && (
          <GroupsCharts stats={stats} filters={filters} />
        )}
      </div>
